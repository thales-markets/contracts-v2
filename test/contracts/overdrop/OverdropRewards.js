...
describe('OverdropRewards', () => {
	...
	describe('Update Merkle Root', () => {
		it('Should allow owner to update merkle root', async () => {
			...
			await expect(overdropRewards.connect(owner).updateMerkleRoot(newMerkleRoot, false, newSeason))
		...
		it('Should revert if non-owner tries to update merkle root', async () => {
			...
			await expect(
				overdropRewards.connect(user1).updateMerkleRoot(newMerkleRoot, false, newSeason)
