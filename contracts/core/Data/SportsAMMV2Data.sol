...
contract SportsAMMV2Data is Initializable, ProxyOwned, ProxyPausable {
    ...
    function getAllActiveGamesAndOpenTickets(
        uint startIndexActiveTickets,
        uint pageSize
    ) external view returns (bytes32[] memory activeGameIds, address[] memory activeTickets) {
        // Get all active tickets from SportsAMMV2Manager
        uint totalActiveTickets = sportsAMM.manager().numOfActiveTickets();

        // Ensure startIndex is within bounds
        if (startIndexActiveTickets < totalActiveTickets) {
            // Calculate the end index
            uint endIndex = startIndexActiveTickets + pageSize;
            if (endIndex > totalActiveTickets) {
                endIndex = totalActiveTickets;
            }

            // Get the specified page of active tickets
            address[] memory allActiveTickets = sportsAMM.manager().getActiveTickets(
                startIndexActiveTickets,
                endIndex - startIndexActiveTickets
            );

            // Temporary arrays to store active tickets and their gameIds
            address[] memory tempActiveTickets = new address[](allActiveTickets.length);
            bytes32[] memory allGameIds = new bytes32[](allActiveTickets.length * 10); // Assuming max 10 games per ticket
            uint activeTicketCount = 0;
            uint totalGameIds = 0;

            // Extract gameIds from active tickets and filter active tickets
                // MarketResult[] memory marketsResult = new MarketResult[](allActiveTickets.length);
            for (uint i = 0; i < allActiveTickets.length; i++) {
                Ticket ticket = Ticket(allActiveTickets[i]);
                bool isActiveTicket = false;
                for (uint j = 0; j < ticket.numOfMarkets(); j++) {
                    (bytes32 gameId, , , , , , , , ) = ticket.markets(j);
                    MarketResult memory marketResult = _getMarketResult(ticket, j);
                    if (marketResult.status == ISportsAMMV2ResultManager.MarketPositionStatus.Open) {
                        allGameIds[totalGameIds] = gameId;
                        totalGameIds++;
                        isActiveTicket = true;
                    }
                }
                if (isActiveTicket) {
                    tempActiveTickets[activeTicketCount] = allActiveTickets[i];
                    activeTicketCount++;
                }
            }

            // Create final array with exact size of active tickets
            activeTickets = new address[](activeTicketCount);
            for (uint i = 0; i < activeTicketCount; i++) {
                activeTickets[i] = tempActiveTickets[i];
            }

            // Count unique gameIds
            uint uniqueGameCount = 0;
            for (uint i = 0; i < totalGameIds; i++) {
                bool isUnique = true;
                for (uint j = 0; j < uniqueGameCount; j++) {
                    if (allGameIds[i] == allGameIds[j]) {
                        isUnique = false;
                        break;
                    }
                }
                if (isUnique) {
                    allGameIds[uniqueGameCount] = allGameIds[i];
                    uniqueGameCount++;
                }
            }

            // Create final array with exact size of unique gameIds
            activeGameIds = new bytes32[](uniqueGameCount);
            for (uint i = 0; i < uniqueGameCount; i++) {
                activeGameIds[i] = allGameIds[i];
            }
        }

        return (activeGameIds, activeTickets);
    }
}
