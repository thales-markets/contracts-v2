...
// internal
import "../proxy/ProxyReentrancyGuard.sol";
import "../proxy/ProxyOwned.sol";
import "../proxy/ProxyPausable.sol";

import "@uniswap/v3-periphery/contracts/interfaces/ISwapRouter.sol";
import "@uniswap/v3-periphery/contracts/interfaces/IQuoter.sol";
import "@uniswap/v3-core/contracts/interfaces/IUniswapV3Pool.sol";
import "@uniswap/v3-core/contracts/interfaces/IUniswapV3Factory.sol";
import "@thales-dao/contracts/contracts/utils/libraries/UniswapMath.sol";

// interfaces
import "@thales-dao/contracts/contracts/interfaces/ICurveSUSD.sol";
import "@thales-dao/contracts/contracts/interfaces/IPriceFeed.sol";
import "@thales-dao/contracts/contracts/interfaces/IPositionalMarketManagerTruncated.sol";
...
contract MockMultiCollateralOnOffRampV1 is Initializable, ProxyOwned, ProxyPausable, ProxyReentrancyGuard {
    ...
    function onrampWithEth(uint amount) external payable nonReentrant notPaused returns (uint convertedAmount) {
        ...
        uint balanceBefore = IERC20(WETH9).balanceOf(address(this));
        WethLike(WETH9).deposit{value: amount}();

        uint balanceDiff = IERC20(WETH9).balanceOf(address(this)) - balanceBefore;
    ...
    function offramp(address collateral, uint amount) external nonReentrant notPaused returns (uint convertedAmount) {
        ...
        IERC20(collateral).safeTransfer(msg.sender, convertedAmount);
    ...
    function setPathPerCollateral(address asset, bytes calldata path, bool doReset) external onlyOwner {
        require(asset != address(0), "Can not set a zero address!");
        if (doReset) {
    ...
    function setPathPerCollateralOfframp(address asset, bytes calldata path, bool doReset) external onlyOwner {
        require(asset != address(0), "Can not set a zero address!");
        if (doReset) {
            bytes memory resetVar;
    ...
    function setMaxAllowedPegSlippagePercentagePerCollateral(
        address collateral,
        uint _maxAllowedSlippagePerCollateral
    ) external onlyOwner {
        require(collateral != address(0), "Can not set a zero address!");
