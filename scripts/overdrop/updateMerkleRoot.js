 ...
 */
async function updateContractMerkleRoot(contractAddress, merkleRoot, newSeason, resetClaims, signer) {
    // Get contract ABI
...
async function main() {
    ...
    try {
        ...
        if (!contractAddress) {
            throw new Error(`OverdropRewards contract address not found for network: ${network} (chainId: ${networkObj.chainId})`);
        }
