...
function verifyAddressProof(address, proofsData) {
	...
	const isValid = verifyProof(normalizedAddress, amount, proof, proofsData.merkleRoot);
...
async function main() {
	...
	if (args.length < 2) {
		...
		console.log('Example:');
		console.log(
			'  node scripts/verifyMerkleProof.js merkle-output/merkleProofs.json 0x742d35Cc6634C0532925a3b8D4aEa4f3eB4FaF88'
		);
		process.exit(1);
	}
