...
function saveTreeData(treeData, outputDir) {
    ...
    fs.writeFileSync(
        path.join(outputDir, 'merkleRoot.json'),
    ...
    fs.writeFileSync(
        path.join(outputDir, 'merkleProofs.json'),
    ...
    fs.writeFileSync(
        path.join(outputDir, 'merkleLeaves.json'),
        JSON.stringify(leavesData, null, 2)
    );
...
async function main() {
    ...
    try {
        ...
        console.log(`Total rewards: ${ethers.formatEther(totalRewards)} ETH (${totalRewards.toString()} wei)`);
