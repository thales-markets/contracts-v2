...
async function main() {
	...
	const sportsAMMV2LiquidityPoolAddress = getTargetAddress(
		'SportsAMMV2LiquidityPoolTHALES',
		network
	);
	...
	const defaultLiquidityProviderDeployed = await upgrades.deployProxy(
		...
		[owner.address, sportsAMMV2LiquidityPoolAddress, thalesCollateralAddress],
	...
	setTargetAddress('DefaultLiquidityProviderTHALES', network, defaultLiquidityProviderAddress);
	await delay(5000);
	...
	setTargetAddress(
		'DefaultLiquidityProviderImplementationTHALES',
		network,
		defaultLiquidityProviderImplementationAddress
	);
	...
	setTargetAddress(
		'DefaultLiquidityProviderProxyAdminTHALES',
		network,
