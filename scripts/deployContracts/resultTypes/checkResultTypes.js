...
async function main() {
	...
	for (let i = 0; i < types.length; i++) {
		...
		try {
			...
			if (Number(typeOnChain) === type.result_type) {
				...
				console.log(
					`[✗] Type ${type.id} (${type.name}): MISMATCH - Expected: ${type.result_type}, Actual: ${typeOnChain}`
				);
			}
		} catch (error) {
			errors.push({
	...
	console.log(
		`[✗] Mismatches: ${mismatches.length} (${((mismatches.length / types.length) * 100).toFixed(
			1
		)}%)`
	);
	console.log(
		`[!] Errors: ${errors.length} (${((errors.length / types.length) * 100).toFixed(1)}%)`
	);
	...
	if (mismatches.length > 0) {
		...
		console.log(
			`npx hardhat run scripts/deployContracts/resultTypes/setResultTypes.js --network ${network}`
		);
	}
	...
	// Print arrays for batch update
	if (mismatches.length > 0) {
		console.log('\n=====================================');
		console.log('BATCH UPDATE ARRAYS');
		console.log('=====================================');

		const marketTypeIds = mismatches.map((m) => m.id);
		const resultTypes = mismatches.map((m) => m.expected);

		console.log('\nmarketTypeIds:');
		console.log(JSON.stringify(marketTypeIds));

		console.log('\nresultTypes:');
		console.log(JSON.stringify(resultTypes));

		console.log(`\nTotal items: ${marketTypeIds.length}`);
		console.log('Both arrays have the same length:', marketTypeIds.length === resultTypes.length);
	}
	...
	if (mismatches.length === 0 && errors.length === 0) {
		...
		console.log(
			`[!] Found ${mismatches.length} mismatches and ${errors.length} errors that need attention.`
		);
